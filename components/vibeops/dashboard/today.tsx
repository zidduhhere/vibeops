...
function MorningBrief({
  ...
  const briefSentence =
    ...
    (decisionCount > 0 ? ` ${decisionCount === 1 ? "One thread needs" : `${decisionCount} threads need`} your decision.` : "") +
    (readyCount > 0 ? ` ${readyCount} ${readyCount === 1 ? "reply is" : "replies are"} ready to send.` : "") +
    (reachCount > 0 ? ` ${reachCount} ${reachCount === 1 ? "person needs" : "people need"} a follow-up today.` : "");
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="relative w-full max-w-[480px] rounded-2xl bg-white px-8 py-7 shadow-2xl text-black">
        ...
        <h2 className="mt-1.5 text-2xl font-bold text-black">{getGreeting()}, {firstName}.</h2>
        ...
        <div className="mt-6 flex items-center justify-between">
          <button type="button" onClick={onDismiss} className="text-xs text-black/40 hover:text-black/60 transition-colors">Dismiss</button>
          <button type="button" onClick={onDismiss}
            ...
            <div key={a.id}>
              <button type="button" onClick={() => setExpanded(isOpen ? null : a.id)}
                ...
                <div className="flex size-8 shrink-0 items-center justify-center rounded-lg border border-border bg-muted">
                  {icon ? <img src={icon.src} alt={icon.alt} className="size-4" /> : <Mail className="size-3.5 text-muted-foreground" />}
                </div>
              ...
              {isOpen && (
                <div className="border-t border-border bg-muted/30 px-5 py-4 space-y-3">
                  <div>
                    ...
                    <p className="text-xs leading-relaxed rounded-xl bg-background border border-border px-3 py-2">{a.clientMessage}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground mb-1">AI draft</p>
                    <p className="text-xs leading-relaxed rounded-xl bg-background border border-border px-3 py-2">{a.aiDraft}</p>
                  </div>
                  {a.status === "pending" && (
                    <div className="flex gap-2">
                      ...
                      <button type="button" className="rounded-xl border border-border px-4 py-2 text-xs font-medium hover:bg-muted transition-colors">Edit draft</button>
                    </div>
                  )}
                  {a.status === "needs-call" && (
                    <div className="flex gap-2">
                      <button type="button" className="rounded-xl bg-primary px-4 py-2 text-xs font-semibold text-primary-foreground hover:bg-primary/90 transition-colors">Review &amp; Reply</button>
                      <button type="button" className="rounded-xl border border-border px-4 py-2 text-xs font-medium text-muted-foreground hover:bg-muted transition-colors">Ignore</button>
                    </div>
                  )}
                </div>
              )}
...
function PriorityQueue({ highlightId }: { highlightId: string | null }) {
  const grouped = groupOrder
    .map((g) => ({ group: g, items: queueItems.filter((q) => q.group === g), config: queueConfig[g] }))
  ...
  return (
    <div className="flex flex-col rounded-2xl border border-border bg-background shadow-sm">
      <div className="flex items-center justify-between border-b border-border px-5 py-4">
        ...
        <span className="flex size-6 items-center justify-center rounded-full bg-primary text-[11px] font-bold text-primary-foreground">{queueItems.length}</span>
      </div>
      <div className="divide-y divide-border">
        {grouped.map(({ group, items, config }) => (
          <div key={group} className="px-5 py-3.5 space-y-2">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{config.countLabel(items.length)}</p>
            {items.map((item) => (
              <div key={item.id} id={`queue-${item.id}`}
                className={cn("rounded-xl border-l-4 bg-muted/40 px-3 py-3 transition-all", config.border, highlightId === item.id && "ring-2 ring-primary ring-offset-1")}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {item.due && (
                      <div className="mt-1.5 flex items-center gap-1 text-[10px] text-muted-foreground">
                        <Clock className="size-3" /><span>{item.due}</span>
                      </div>
                    )}
                  </div>
                  {group !== "coming-up" && (
                    <button type="button" className="shrink-0 rounded-lg border border-border bg-background px-3 py-1.5 text-xs font-medium transition-colors hover:bg-muted">
...
export function TodayView({ userName, userPicture }: { userName?: string; userPicture?: string }) {
  ...
  return (
    <>
      {showBrief && <MorningBrief firstName={firstName} onDismiss={dismissBrief} onChipClick={handleChipClick} />}
