...
const AUTOMATIONS = [
  {
    id: "1",
    key: "auto_draft_pricing",
    name: "Auto-draft pricing replies",
    description:
      "When a Gmail thread contains pricing keywords, AI drafts a discovery-first reply for your review.",
    category: "Leads",
    triggerType: "email",
  },
  {
    id: "2",
    key: "scope_creep_detector",
    name: "Scope creep detector",
    description:
      "Flags any client message requesting work outside the original proposal scope before replying.",
    category: "Clients",
    triggerType: "email",
  },
  {
    ...
    description:
      "If a proposal hasn't received a reply in 3 days, remind you and offer a draft nudge.",
    category: "Leads",
    triggerType: "time",
  },
  {
    ...
    description:
      "Every Friday, draft a status update for each active client based on Notion task notes.",
    category: "Clients",
    triggerType: "time",
  },
  {
    ...
    description:
      "7 days after an invoice is sent with no payment, draft a polite reminder email.",
    category: "Finance",
    triggerType: "time",
  },
  {
    ...
    description:
      "Analyse first messages from new contacts and score them on intent, budget signals, and scope clarity.",
    category: "Leads",
    triggerType: "email",
  },
];
...
const TRIGGER_PARAM_FIELDS: Record<
  string,
  { label: string; field: string; type: "tags" | "number" | "day" }[]
> = {
  auto_draft_pricing: [
    { label: "Trigger keywords", field: "keywords", type: "tags" },
  ],
  proposal_followup: [
    { label: "Days without reply", field: "delay_days", type: "number" },
  ],
  invoice_reminder: [
    { label: "Days after invoice sent", field: "days_after", type: "number" },
  ],
};
...
export function AutomationsView() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      if (automationsRes.automations) {
        ...
        automationsRes.automations.forEach(
          (a: { enabled: boolean; automation_key: string }) => {
      ...
      if (configsRes.configs) {
        ...
        configsRes.configs.forEach((c: AutomationConfig) => {
          map[c.automation_key] = c;
        });
        setConfigs(map);
      }
  ...
  const runNow = async () => {
    ...
    try {
      ...
      alert(
        data.success
          ? `Triggered for: "${data.triggered_for}"`
          : (data.message ?? "No matching email found"),
      );
      await fetchAll();
    } finally {
      setRunningNow(false);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            AI rules running in the background on your behalf.
          </p>
        </div>
          ...
          <button
            ...
            className={cn(
              ...
              filter === c
                ...
                : "text-muted-foreground hover:bg-muted border border-border bg-background",
            )}
          >
            {c}
          </button>
        ...
        {filtered.map((automation) => {
          ...
          return (
            <div
              key={automation.id}
              className={cn(
                "rounded-2xl border border-border bg-background p-5 shadow-sm transition-opacity",
                !isActive && "opacity-60",
              )}
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <button
                  type="button"
                  onClick={() => toggleAutomation(automation.key, isActive)}
                  className={cn(
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={() => openPanel(automation.key)}
            ...
            <div className="flex items-start justify-between border-b border-border p-6">
              <div>
                <h2 className="font-semibold text-lg">
                  {panelAutomation.name}
                </h2>
                <p className="mt-1 text-sm text-muted-foreground">
                  {panelAutomation.description}
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="flex-1 space-y-6 p-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={() =>
                    toggleAutomation(panelKey, activeKeys.has(panelKey))
                  }
                  className={cn(
                  ...
                  {(["agentic", "supervised"] as const).map((m) => (
                    ...
                    >
                      <p className="font-medium text-sm">
                        {m === "agentic" ? "🤖 Agentic" : "👁️ Supervised"}
                      </p>
                ...
                <div key={field.field}>
                  ...
                  {field.type === "tags" && (
                    <input
                      ...
                      value={(
                        (form.trigger_params[field.field] as string[]) ?? []
                      ).join(", ")}
                      onChange={(e) =>
                        setForm((f) => ({
                          ...f,
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, gmail_filter: e.target.value }))
                  }
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                />
              </div>
      ...
      {/* Run detail modal */}
      {selectedRun && (
        <div className="fixed inset-0 z-Also once a mail is opened and a draft is created it should be stored in the database, to avoid constatn ai usage and also the AI drafting should be more customisable, just showign some text is not enough, it should show a shimmering animation and ask tone in tiles and a proper structure should be shown, like the proper gmail with proper structure60 flex items-center justify-center p-4">
          <div
          ...
          <div className="relative z-10 w-full max-w-md rounded-2xl bg-background p-6 shadow-2xl">
            <div className="flex items-start justify-between mb-4">
              <div>
                <p className="font-semibold text-sm">
                  {selectedRun.trigger_email_subject}
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  {selectedRun.trigger_email_from}
                </p>
              </div>
              <button
                type="button"
            ...
            {selectedRun.status === "held_for_review" && (
              <button
                type="button"
                onClick={() => sendRun(selectedRun.id)}
                disabled={sending}
